...
const PlanNNLVSL = () => {
  ...
  const [planNNLBanner, setPlanNNLBanner] = useState([]);
  ...
  const showPlanNNLBanner = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanNNLBanner;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  return (
    <>
      <section className="planzero_section ">
        ...
        >
          {planNNLBanner.slice(0, 1).map((banner) => {
