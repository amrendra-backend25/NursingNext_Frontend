...
const PlanZero = () => {
  ...
  const showPlanUGVideos = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanZeroVideo;
  ...
  const showPlanUGStats = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanZeroStats;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  const showPlanUGSalient = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanZeroSalient;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  const showPlanUGBanner = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanZeroBanner;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  const showPlanUGOfferTab = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanZeroOffersPrice;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
          ...
          <div className="plan_button_section_parent">
                  ...
                  {planUGPriceTab.slice(0, 3).map((tab, index) => {
                    ...
                    return (
                      <>
                        ...
                        >
                          {planName} {planValidity} | ₹ {planActualPrice}
                        </button>
                      </>
                    );
                  })}
            ...
            <div className="plan_tab_button_right_1">
              {planUGPriceTab.slice(0, 3).map((TabDesc, index) => {
                ...
                yearDescription1;
                const sentences = yearDescription1.split(". ");
                return (
                  <>
                    {activeTab === index + 1 && (
