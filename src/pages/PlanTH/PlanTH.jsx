...
const PlanTH = () => {
  ...
  const [planTHBanner, setPlanTHBanner] = useState([]);
  ...
  const showPlanTH = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanTHBanner;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  return (
    <>
      <section className="planzero_section ">
        ...
        >
          {planTHBanner.slice(0, 1).map((banner) => {
