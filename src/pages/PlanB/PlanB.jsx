...
const PlanB = () => {
  ...
  const [planBBanner, setPlanBBanner] = useState([]);
  ...
  const showPlanBBanner = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanBBanner;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPlanBBanner(planABannerData);
      } catch (error) {
        setIsError(error.msg);
      }
    };
  ...
  return (
    <>
      <section className="planzero_section ">
        ...
        >
          {planBBanner.slice(0, 1).map((banner) => {
