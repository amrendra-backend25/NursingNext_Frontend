...
const PlanA = () => {
  ...
  const [planABanner, setPlanABanner] = useState([]);
  ...
  const showPlanABanner = async () => {
    try {
      const response = await Paths.EndpointsURL.PlanABanner;
      const record = await axios.get(response, {
        headers: {
          "Content-type": "application/json",
        },
  ...
  return (
    <>
      <section className="planzero_section ">
        ...
        >
          {planABanner.slice(0, 1).map((banner) => {
